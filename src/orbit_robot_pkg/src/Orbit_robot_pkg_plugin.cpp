    ...
    public: void OnUpdate()
    {
      ...
      if (this->now_secs > 600.0)
      {
        ...
        for (auto &workingSatellite : orbitLinkVector)
        {
          ...
          workingSatellite.SetOrbitPosition(position.Pos().X(), position.Pos().Y(), position.Pos().Z() );
          ...
          workingSatellite.SetOrbitVelocity(velocity.X(),velocity.Y(),velocity.Z());
